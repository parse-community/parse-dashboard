...
export default class ParseApp {
  constructor(params) {
    this.name = params.name || params.appName;
    this.createdAt = params.created_at ? new Date(params.created_at) : new Date();
    this.applicationId = params.key || params.applicationId;
    this.slug = params.friendly_id;
    if (!this.slug && params.dashboardURL) {
      let pieces = params.dashboardURL.split('/');
      this.slug = pieces[pieces.length - 1];
    }
    this.clientKey = params.secret || params.clientKey;
    this.javascriptKey = params.javascript_key || params.javascriptKey;
    this.masterKey = params.push_key || params.masterKey;
    this.restKey = params.rest_api_key || params.restKey;
    this.windowsKey = params.windows_key || params.windowsKey;
    this.webhookKey = params.webhook_key || params.webhookKey;
    this.fileKey = params.api_key || params.apiKey;
    this.production = !!params['is_production?'];
    this.serverURL = params.server_url || 'https://api.parse.com/1';
  ...
  removeCollaboratorById(id) {
    ...
    promise.then(() => {
      //TODO(drewgross): this currently works because everything that uses collaborators
  ...
  setRequireRevocableSessions(require) {
    ...
    promise.then(() => {
      //TODO(drewgross): this currently works because everything that uses this
