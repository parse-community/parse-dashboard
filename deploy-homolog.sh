...
ssh -t -i $b4a_certs_path/$pem $user@$host "sudo su back4app -c 'cd ~/scm/parse-dashboard && sed -i \"s/http:\/\/localhost:4000\/parseapi/https:\/\/dashboard-homolog.back4app.com\/parseapi/\" node_modules/parse/lib/browser/settings.js'"
...
#ssh -t -i $b4a_certs_path/$pem $user@$host "sudo su back4app -c 'sed -i \"s/http:\/\/localhost:4000\/parseapi/https:\/\/dashboard-homolog.back4app.com\/parseapi/\"  && npm run prepublish && npm run build'"
