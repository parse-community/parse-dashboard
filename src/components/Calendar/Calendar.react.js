...
export default class Calendar extends React.Component {
  ...
  renderMonth() {
    return (
      <div className={styles.month}>
        <a role='button' onClick={this.handlePrev.bind(this)} />
        <a role='button' onClick={this.handleNext.bind(this)} />
  ...
  renderDays() {
    ...
    for (let i = 1; i <= days; i++) {
      ...
      labels.push(
        <a role='button' key={'day' + i} className={className} onClick={onChange}>{i}</a>
      );
    }
