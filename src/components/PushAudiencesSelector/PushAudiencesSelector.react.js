...
const PushAudiencesSelector = ({
  ...
  <div className={styles.container}>
    ...
    <div className={styles.header}>
      <div className={[styles.cell, styles.col1].join(' ')}>Audience</div>
      {AUDIENCE_SIZE_FETCHING_ENABLED ? (
        <div className={[styles.cell, styles.col2].join(' ')}>Size</div>
      ) : null}
      {AUDIENCE_CREATED_DATE_ENABLED ? (
        <div className={[styles.cell, styles.col3].join(' ')}>Created</div>
      ) : null}
    </div>
    {children}
  </div>
