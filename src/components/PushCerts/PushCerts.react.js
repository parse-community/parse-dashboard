...
class PushCerts extends React.Component {
  ...
  render() {
    const renderTable =
      ...
      (this.props.certs && this.props.certs.length > 0);
    return (
      <Fieldset
        legend="Apple Push Certificates"
        description={
          'You may upload up to 6 push certificates. This limit cannot be raised.'
        }
      >
        <Field
          labelWidth={60}
          label={
            <Label
              ...
              description="Upload your .p12 certificate file to enable push for iOS and OS X."
            />
          }
          input={
            <FileInput
        ...
        {renderTable || this.props.error ? (
          <FormNote color="orange" show={!!this.props.error}>
            {this.props.error}
          </FormNote>
        ) : null}
        {renderTable ? (
          <Field
            ...
            label={
              <Label
                ...
                description="Remember, you can only upload two certificates per bundle."
              />
            }
            input={<CertsTable {...this.props} />}
          />
        ) : null}
      </Fieldset>
    );
