...
function PushAudiencesStore(state, action) {
  ...
  switch (action.type) {
    case ActionTypes.FETCH:
      ...
      const promise = action.app.apiRequest('GET', path, {}, { useMasterKey: true });

      return promise.then(({ results, showMore }) => {
        return Map({ lastFetch: new Date(), audiences: List(results), showMore: showMore});
      });
    case ActionTypes.CREATE:
      return action.app.apiRequest('POST', 'push_audiences', { query: action.query, name: action.name, }, { useMasterKey: true })
          .then(({ new_audience }) => {
    ...
    case ActionTypes.DESTROY:
      return action.app.apiRequest('DELETE', `push_audiences/${action.objectId}`, {}, { useMasterKey: true })
          .then(() => {
            return state.update('audiences',(audiences) => {
