...
let AppCard = ({
  ...
  let versionMessage = app.serverInfo.error ?
    <div className={styles.serverVersion}>{t('Server not reachable')}: <span className={styles.ago}>{t(app.serverInfo.error.toString())}</span></div> :
    <div className={styles.serverVersion}>
      {t('Server URL')}: <span className={styles.ago}>{app.serverURL || 'unknown'}</span>
