...
export default class UsersSettings extends DashboardView {
  ...
  renderContent() {
    const toolbar = <Toolbar section='Settings' subsection='Users' />
    ...
    const renderForm = ({fields, setField}) => {
      const userSessionsFields = <Fieldset
      ...
      const userAuthenticationFields = <Fieldset
      ...
      const socialLoginFields = <Fieldset
        legend='Social Login'
    ...
    return <FlowView
      ...
      onSubmit={({ changes, setField }) => {
        ...
        if (changes.facebookAppID !== undefined && changes.facebookAppSecret !== undefined) {
          const fbAppPromise = this.context.addConnectedFacebookApp(changes.facebookAppID, changes.facebookAppSecret);
          fbAppPromise.then(() => {
            setField('facebookAppID', '');
            setField('facebookAppSecret', '');
          })
        ...
        if (changes.twitterConsumerKey !== undefined) {
          const twitterAppPromise = this.context.addConnectedTwitterApp(changes.twitterConsumerKey);
